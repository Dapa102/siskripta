...
class TugasAkhirResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('mahasiswa.name')->label('Mahasiswa')->searchable()->visible(auth()->user()->role !== 'mahasiswa'),
                Tables\Columns\TextColumn::make('judul')->searchable()->wrap(),
                Tables\Columns\BadgeColumn::make('status')->colors(['warning' => 'pending', 'success' => 'acc', 'danger' => 'reject', 'primary' => 'revisi']),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('review')
                    ...
                    ->form([
                        Forms\Components\Select::make('status')->options(['acc' => 'Terima (ACC)', 'reject' => 'Tolak', 'revisi' => 'Revisi'])->required(),
                        Forms\Components\Textarea::make('komentar')->required(),
                    ])
                    ->action(function (Submission $record, array $data) {
                        if ($data['status'] === 'revisi') $record->increment('revisi_count');
    ...
    public static function shouldRegisterNavigation(): bool { return auth()->user()->role !== 'super_admin'; }
    public static function canViewAny(): bool { return auth()->user()->role !== 'super_admin'; }
    public static function canCreate(): bool { return auth()->user()->role === 'mahasiswa'; }
    
    public static function canView(\Illuminate\Database\Eloquent\Model $record): bool {
        $user = auth()->user();
        if ($user->role === 'super_admin') return true;
        if ($user->role === 'mahasiswa') return $record->mahasiswa_id === $user->id;
        if ($user->role === 'dosen') return \App\Models\Bimbingan::where('dosen_id', $user->id)->where('mahasiswa_id', $record->mahasiswa_id)->exists();
        return false;
    }

    public static function canEdit(\Illuminate\Database\Eloquent\Model $record): bool {
        $user = auth()->user();
        if ($user->role === 'mahasiswa' && $record->mahasiswa_id === $user->id) return in_array($record->status, ['pending', 'revisi', 'reject']);
        return false;
    }
